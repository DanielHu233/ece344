...
main(int argc, char **argv)
{
	 ...
	 * mycontext.uc_mcontext.gregs[]? */
        ...
        //are on teh stack. grow down. RBP-x
        
        //setcontext_called is local variable on stack, hence accessed through decrementing fp
	...
	printf("number of bytes pushed to the stack between setcontext_called "
	       "and err = %ld\n", (unsigned long)((int*)(mycontext.uc_mcontext.gregs[REG_RBP] - 8) - 
                (int*)(mycontext.uc_mcontext.gregs[REG_RBP] - 16)));
	...
	printf("number of bytes between err and the saved stack in mycontext "
	       "= %ld\n", (unsigned long)((int*)(mycontext.uc_mcontext.gregs[REG_RBP] - 16)
                - (int*)mycontext.uc_mcontext.gregs[REG_RSP]));
